...
const updateCardsToShow = ()=>{
  ...
  return (
    ...
    w-full overflow-hidden'id='projects'>
      ...
      <p className='text-center text-gray-500 mb-8 max-w-80nmx-auto'>
        Crafting Spaces, Building Legacies-Explore Our Portfolio
      </p>
      {/* slider button */}
      <div className='flex justify-end items-center mb-8'>
        <button className='p-3 bg-gray-200 rounded mr-2' aria-label='previous project'>
          <img onClick={prevProject} src={assets.arrow_left} alt='previous' /></button>
        <button className='p-3 bg-gray-200 rounded mr-2' aria-label='next project'>
          <img onClick={nextProject} src={assets.arrow_right} alt='next' /></button>
      </div>
        ...
        <div className='flex gap-8 transition-transform duration-500
          ...
          {projectData.map((project, index) => (
            <div key={index} className='relative flex-shrink-0 w-full sm:w-1/4'>
              ...
              <img className='w-2x1 h-80 mb-14 ' src={project.image} alt={project.title} />
              <div className='absolute left-0 right-0 bottom-5 flex justify-center'>
                <div className='inline-block bg-white w-3/4 px-4 py-2 shadow-md'>
                  ...
                  <p className='text-gray-500 text-sm'>
                    {project.price} <span className='p-x1'>|</span> {project.location}
                  </p>
                </div>
              </div>
            </div>
